...
def build_ppt(src: Path, out_name="spout_position_decoder_summary.pptx") -> Path:
    ...
    cm = src / "locanmf_cross_mouse_comparison_0606-0807.png"
    if cm.exists():
        s = prs.slides.add_slide(BLANK)
        title(s, "CROSS-MOUSE: cortical representation of spout position (6/6-8/7)",
    ...
    wac = src / "locanmf_within_animal_consistency_0606-0807.png"
    if wac.exists():
        s = prs.slides.add_slide(BLANK)
        title(s, "WITHIN-ANIMAL consistency of per-position decode/encode across sessions (6/6-8/7)",
    ...
    rsa = src / "locanmf_rsa_sessions_0606-0807.png"
    if rsa.exists():
        s = prs.slides.add_slide(BLANK)
    ...
    rsr = src / "locanmf_rsa_rdms_0606-0807.png"
    if rsr.exists():
        s = prs.slides.add_slide(BLANK)
    ...
    hsum = src / "locanmf_rsa_hemisphere_summary_0606-0807.png"
    if hsum.exists():
        s = prs.slides.add_slide(BLANK)
    ...
    hrdm = src / "locanmf_rsa_hemisphere_rdms_0606-0807.png"
    if hrdm.exists():
        s = prs.slides.add_slide(BLANK)
